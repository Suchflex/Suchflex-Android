...
package com.qactivo.distribit.boinc.rpc;
...
import com.qactivo.distribit.boinc.utils.BOINCUtils;
...
public class RpcClient {
	...
	public boolean open(String socketAddress) {
		if (isConnected()) {
			// Already connected
			if(com.qactivo.distribit.boinc.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "Attempt to connect when already connected");
		...
		try {
			...
			if(com.qactivo.distribit.boinc.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "connect failure: illegal argument", e);
			mSocket = null;
			return false;
		}
		catch (IOException e) {
	...
	public synchronized boolean projectOp(int operation, String projectUrl) {
		try {
			...
			case PROJECT_RESET:
				...
				break;
			default:
				if(com.qactivo.distribit.boinc.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "projectOp() - unsupported operation: " + operation);
				return false;
			}
			String request =
				"<" + opTag + ">\n" +
	...
	public synchronized boolean transferOp(int operation, String projectUrl, String fileName) {
		try {
			...
			case TRANSFER_ABORT:
				...
				break;
			default:
				if(com.qactivo.distribit.boinc.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "transferOp() - unsupported operation: " + operation);
				return false;
			}
			mRequest.setLength(0);
			mRequest.append("<");
	...
	public boolean resultOp(int operation, String projectUrl, String resultName) {
		try {
			...
			case RESULT_ABORT:
				...
				break;
			default:
				if(Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "resultOp() - unsupported operation: " + operation);
				return false;
			}
			mRequest.setLength(0);
			mRequest.append("<");
